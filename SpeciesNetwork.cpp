...
//        network[geneName_to_uniqId[str_id1]][geneName_to_uniqId[str_id2]] = edge_weight; // store edge weight between gene1 and gene2 in network adj matrix
        ...
        node_adjList[geneName_to_uniqId[str_id1]].push_back(
                geneName_to_uniqId[str_id2]
                                                           ); // store gene2 in the adj list of gene1, only do this for gene1 assuming that network file has both edges of: gene1->gene2 and gene2->gene1
    }
    fclose(F); // close file
